...
async function main() {
  console.log(`Seeding local database with ${skills.length} skills...`);
  ...
  for (let i = 0; i < skills.length; i += batchSize) {
    ...
    try {
      execSync(`npx wrangler d1 execute agentskills-db --local --file=./seed_batch_${i}.sql`, {
        cwd: apiDir,
        stdio: 'pipe',
        timeout: 60000
      });
  ...
  // Verify
  try {
    const result = execSync(`npx wrangler d1 execute agentskills-db --local --command="SELECT COUNT(*) as count FROM skills"`, {
      cwd: apiDir,
      encoding: 'utf8'
    });
